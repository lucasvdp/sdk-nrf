...
void main(void)
{
	...
	err = pdn_cid_create(&cid, pdn_event_handler);
	if (err) {
		printk("pdn_cid_create() failed, err %d\n", err);
		return;
	}
	...
	err = pdn_cid_configure(cid, apn, PDN_FAM_IPV4V6, NULL);
	if (err) {
		printk("pdn_cid_configure() failed, err %d\n", err);
		return;
	}
	...
	printk("PDP Context %d (cid)\t PDN ID %d\n", 0, pdn_id_get(0));
	printk("PDP Context %d (cid)\t PDN ID %d\n", cid, pdn_id_get(cid));
