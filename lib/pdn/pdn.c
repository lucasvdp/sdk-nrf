...
{
	for (size_t i = 0; i < CONFIG_PDN_CONTEXTS_MAX; i++) {
...
static void at_notif_handler(void *context, const char *notif)
{
	...
	if (p) {
		esm_err = atoi(p + strlen("CNEC_ESM: "));
...
int pdn_init(void)
{
	...
	return at_notif_register_handler(NULL, at_notif_handler);
}
...
int pdn_cid_create(uint8_t *cid, pdn_event_handler_t cb)
{
	int err;
	char *p;
	struct pdn *pdn;
...
int pdn_cid_configure(uint8_t cid, const char *apn, enum pdn_fam fam,
		      struct pdn_pdp_opt *opt)
{
	...
	if (opt) {
		 ...
		 * Only format these params when `opt` is passed explicitely.
		 */
...
int pdn_cid_auth_set(uint8_t cid, enum pdn_auth method,
...
int pdn_cid_destroy(uint8_t cid)
{
	int err;
	struct pdn *pdn;
...
static int pdn_sys_init(const struct device *unused)
{
	...
	err = pdn_cid_configure(0, CONFIG_PDN_DEFAULT_APN,
	...
	err = pdn_cid_auth_set(0, CONFIG_PDN_DEFAULT_AUTH,
